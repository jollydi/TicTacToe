...
class MainActivity : AppCompatActivity() {
    ...
    private val tttButtons: Array<Array<Button?>> = Array(TicTacToeGame.NUM_ROWS) {
        arrayOfNulls<Button>(TicTacToeGame.NUM_COLUMNS)
    }
        ...
        for (row in 0 until TicTacToeGame.NUM_ROWS) {
            for (col in 0 until TicTacToeGame. NUM_COLUMNS) {
                ...
                tttButtons[row][col]?.setOnClickListener {
                    ...
                    updateView()
                    Log.d(Constants.TAG, "Pressed button at ($row, $col)")
                }
            }
        }
    }
